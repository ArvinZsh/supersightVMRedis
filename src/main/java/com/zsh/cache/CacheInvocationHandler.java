...
public class CacheInvocationHandler implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		for (Map.Entry<String, CacheHandler> interceprter : heandler.entrySet()) {
			CacheFilterMethodName filterMethodNames = interceprter.getValue().getClass()
					.getAnnotation(CacheFilterMethodName.class);

			// 1. 对应接口类
			// 2. 对应方法名
			if (method.getDeclaringClass().getName().equals(filterMethodNames.type().getName())
					&& filterMethodNames.methodName().contains(method.getName())) {
				// 走缓存
